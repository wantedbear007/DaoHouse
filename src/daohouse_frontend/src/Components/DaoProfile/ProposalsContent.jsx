import React, { useState, useEffect } from "react";
import Card from "../Proposals/Card";
import { Link, useParams } from "react-router-dom";
import { useAuth } from "../../Components/utils/useAuthClient";
import nodata from "../../../assets/nodata.png";

const ProposalsContent = ({ proposals, isMember }) => {
  const { createDaoActor } = useAuth();
  const { daoCanisterId } = useParams();
  const [searchTerm, setSearchTerm] = useState("");
  const [fetchedProposals, setFetchedProposals] = useState([]);

  const allProposals = Array.isArray(proposals) ? proposals : [];

  const handleSearchChange = (event) => {
    setSearchTerm(event.target.value);
  };

  const getProposals = async () => {
    if (searchTerm.trim() === "") {
      setFetchedProposals([]);
      return;
    }

    try {
      const daoActor = createDaoActor(daoCanisterId);
      const response = await daoActor.search_proposal(searchTerm);
      setFetchedProposals(response);
    } catch (error) {
      console.error("Error fetching proposals:", error);
    }
  };

  useEffect(() => {
    getProposals();
  }, [searchTerm, createDaoActor, daoCanisterId]);

  const displayedProposals = searchTerm.trim() === "" ? allProposals : fetchedProposals;

  return (
    <div className="mt-6 mb-6">
      {/* <div className="flex items-center justify-between">
        <h1 className="lg:text-[24px] md:text-[18px] text-[16px] font-bold">
          Proposals
        </h1>
        {isMember && (
          <Link
            to="/create-proposal"
            className="flex justify-center items-center text-[16px] relative lg:w-[220px] lg:h-[50px] md:w-[185px] md:h-[46px] w-[30px] h-[30px] bg-white rounded-full shadow-lg"
          >
            <span className="md:absolute md:text-[35px] text-[28px] font-thin lg:left-5 lg:bottom-[1px] md:left-4 md:bottom-[0.1%] mb-[20%]">+</span>
            <span className="ml-6 hidden md:block">Create Proposal</span>
          </Link>
        )}
      </div> */}
      <div className="bg-transparent pt-3 pb-8 mt-4 mb-8 rounded- hidden md:block">
        <div className="flex justify-between items-center px-6 mb-3">
          <span className="text-[20px] text-[#05212C] font-semibold">All</span>
          <div className="flex-grow flex justify-center px-6 mx-2">
            <SearchProposals
              onChange={handleSearchChange}
              onClick={getProposals}
              width="100%"
              bgColor="transparent"
              placeholder="Search by proposal ID"
              className="border-2 border-[#AAC8D6] w-full max-w-lg"
            />
          </div>
        </div>
        <div className="w-full border-t py-6 px-4 border-[#0000004D] rounded-[10px] mb-4">
          <div className="bg-transparent rounded flex flex-col gap-8">
            {displayedProposals.length === 0 ? (
              <p className="text-center font-black">
                <img src={nodata} alt="No Data" className="mx-auto block" />
              </p>
            ) : (
              displayedProposals.map((proposal, index) => (
                <Card key={index} proposal={proposal} />
              ))
            )}
          </div>
        </div>
      </div>

      <div className="bg-white md:hidden rounded-[10px] mt-4 mb-8 flex flex-col gap-8">
        {displayedProposals.length === 0 ? (
          <p className="text-center font-black">
            <img src={nodata} alt="No Data" className="mb-1 mx-auto block" />
          </p>
        ) : (
          displayedProposals.map((proposal, index) => (
            <Card key={index} proposal={proposal} />
          ))
        )}
      </div>
    </div>
  );
};

export default ProposalsContent;

export const SearchProposals = ({
  width,
  bgColor,
  placeholder,
  className,
  onChange,
  ...inputProps
}) => (
  <div
    className={`flex items-center p-2 bg-${bgColor} rounded-full max-w-md mx-auto ${className}`}
    style={{ minWidth: width }}
  >
    <div className="mx-3">
      <svg
        width="26"
        height="26"
        viewBox="0 0 26 26"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M25 25L19.2094 19.2094M19.2094 19.2094C20.1999 18.2189 20.9856 17.043 21.5217 15.7488C22.0577 14.4547 22.3336 13.0676 22.3336 11.6668C22.3336 10.266 22.0577 8.87896 21.5217 7.5848C20.9856 6.29065 20.1999 5.11475 19.2094 4.12424C18.2189 3.13373 17.043 2.34802 15.7488 1.81196C14.4547 1.27591 13.0676 1 11.6668 1C10.266 1 8.87896 1.27591 7.5848 1.81196C6.29065 2.34802 5.11474 3.13373 4.12424 4.12424C2.12382 6.12466 1 8.8378 1 11.6668C1 14.4958 2.12382 17.209 4.12424 19.2094C6.12466 21.2098 8.8378 22.3336 11.6668 22.3336C14.4958 22.3336 17.209 21.2098 19.2094 19.2094Z"
          stroke="#646464"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </div>
    <input
      onChange={onChange}
      type="text"
      placeholder={placeholder}
      className="pl-4 pr-10 py-2 w-full bg-transparent focus:outline-none placeholder-zinc-400 text-zinc-700"
      {...inputProps}
    />
  </div>
);

