            ...
            <div className="w-full">
                ...
                <div className="flex gap-4 text-xs py-2 px-6  bg-[#AAC8D6] rounded-b-lg">
                    {gridItems.map((item, index) => (
                        <div key={index} className="text-[16px] text-black py-2 rounded-md text-nowrap">
                            ...
                            <div className="text-nowrap	py-1 text-[16px] mx-2">
                                ...
                                {item.time && <span className="text-[#55646b] mx-1 text-[12px]"> {item.time}</span>}
                            </div>
                        </div>
                    ))}
                    <div className={`w-fit  ${proposal.status === "Rejected" ? "bg-[#D85032]" :
                        proposal.status === "Approved" ? "bg-[#4FB565]" :
                            proposal.status === "In Progress" ? "bg-[#4993B0]" : ""
                        } ml-auto text-white text-xs font-semibold rounded-full my-4 mx-4 px-3 py-1 inline-block`}> <span className="text-[#34342a] text-[16px]">

                            {proposal.status === "In Progress" &&
                                <span>• •</span>
                            }
                        </span> {proposal.status}</div>
                </div>

                <div className="p-4">
                    <div className='flex flex-col right-card flex-1'>
                        <div className="flex items-center justify-start ">
                            <div className="font-bold text-xl text-[#229ED9]  mb-2 p-2">Transfer</div>
                            <span className="border-r-2 h-6 border-[#229ED9] mx-2" />
                            <div className="font-semibold text-[16px] text-[#229ED9] mb-2 p-2">Proposal ID: #{proposal.id}</div>
                        </div>
