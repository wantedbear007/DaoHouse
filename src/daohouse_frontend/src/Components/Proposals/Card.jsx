...
export default function Card({ proposal, resData }) {

  console.log(resData, "all data ")
  const a= resData?.proposal_description;
 ...
 console.log("A",a)
  ...
  const bigIntValue2 = BigInt(resData?.share_count || 0);
  const sharecount = Number(bigIntValue2);
  console.log("share",sharecount)
  const bigIntValue3 = BigInt(resData?.required_votes
 || 0);
  const votecount = Number(bigIntValue3);
  console.log("like",votecount)
  const bigIntValue4 = BigInt(resData?.comments|| 0);
  ...
  const gridItems = [
    { label: "Submitted On:", value: resData?.
      proposal_submitted_at, time: "5:32:11"  },
    { label: "Expires On:", value: resData?.proposal_expired_at
      , time: "5:32:11"},
    { label: "Votes Required:", value: resData?.required_votes }
  ];

  // console.log("-----g",gridItems)
  // Animation options for the approved

    const bigIntValue = BigInt(resData?.proposal_approved_votes || 0);
    const approvedvoters = Number(bigIntValue);
    console.log("???",approvedvoters)

    const bigIntValue1 = BigInt(resData?.proposal_rejected_votes || 0);
    const rejectedvoters = Number(bigIntValue1);
    console.log("?",rejectedvoters)
    
 


  const principalString = resData?.created_by
  ? Principal.fromUint8Array(new Uint8Array(resData.created_by)).toText()
  : "Unknown";
    ...
    <div className="rounded-l-lg rounded-t-lg w-full  shadow-md flex">
      <div className="w-full">
        ...
        <div className="bg-[#000000] flex items-center justify-between w-full rounded-t-lg">
          <div className="flex p-3  border border-red-600">
        ...
        <div className="p-4">
          <div className="flex flex-col right-card flex-1 relative">
            <div className="flex items-center justify-start ">
              ...
              <div className="font-semibold md:text-[16px] text-[14px] text-[#229ED9] mb-2 p-2 truncate ... w-20">
                Proposal ID: #{resData?.proposal_id}
              </div>
            </div>

           {resData?.proposal_description && (
              <div
                className="mx-4 text-black text-[12px] md:text-[16px] font-normal my-3"
                dangerouslySetInnerHTML={{ __html: resData?.proposal_description }}
              />
            )}
          ...
          <div className="p-2 md:hidden">
            <div className="flex items-center space-x-8">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-base text-black">{approvedvoters} votes</span>
              </div>
              <div className="flex items-center space-x-3">
