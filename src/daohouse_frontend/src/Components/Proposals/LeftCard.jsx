...
export default function LeftCard() {
    ...
    return (
        <div className="rounded-l-lg rounded-r-lg w-full  shadow-md flex">
            <div className="w-full">
                <div className="bg-[#000000] flex items-center justify-between w-full rounded-tl-lg">
                ...
                <div className="flex flex-col bg-[#AAC8D6] rounded-bl-lg pb-5 pr-2">
                    <div className=" w-fit bg-[#4993B0] text-white text-xs font-semibold rounded-full my-4 mx-4 px-3 py-1 inline-block"> <span className="text-[#34342a] ">• •</span> In Progress</div>
                    <div className="grid grid-cols-3 gap-4 text-xs px-6 my-2">
                        {gridItems.map((item, index) => (
                            <div key={index} className="text-[12px] text-black py-2 rounded-md text-nowrap">
                                <span className="font-bold text-[12px] text-nowrap	"> • {item.label}</span><br />
                                <div className="text-nowrap	py-1 text-[12px] mx-2">
                                    <span className=" text-[12px]">    {item.value}</span>

                                    {item.time && <span className="text-[#55646b] mx-1 text-[10px]"> {item.time}</span>}
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="mx-2  dark:border-zinc-700 pt-2 flex justify-between text-zinc-500 dark:text-zinc-400 px-3">
                        {buttons.map((button, index) => (
                            <button key={index} className="flex items-center space-x-2">
                                {button.icon}
                                <span className="text-black text-[12px]">{button.text}</span>
                            </button>
                        ))}
                    </div>
                </div>
